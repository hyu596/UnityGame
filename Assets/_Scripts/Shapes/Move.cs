...
public class Move : MonoBehaviour {
    ...
    private bool IsAbleTomove(Vector2 loce)
    {
        return ((int)loce.y >= RandomManager.r1);
    }
	...
	protected void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
            ...
            if (hit.collider != null && hit.collider.transform == this.transform && IsAbleTomove(origin_place))
			{
				still_moving = true;
		...
		if (!still_moving && checkForValid()) {
			...
			if (y == -1) {
				...
				{
						RandomManager.Move2ndLineUp();
						RandomManager.SpawnNextShape(RandomManager.lin1_2);
						RandomManager.SpawnNextShape(RandomManager.lin2_2);
						RandomManager.SpawnNextShape(RandomManager.lin3_2);
				}
